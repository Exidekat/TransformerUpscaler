...
def default_conv(in_channels, out_channels, kernel_size, bias=True, groups=1):
    ...
    conv = nn.Conv2d(in_channels, out_channels, kernel_size, padding=(kernel_size // 2), bias=bias, groups=groups)
    return wn(conv)
...
class BasicConv(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, kernel_size, stride=1, padding=1, dilation=1,
        ...
        if fan:
            ...
            self.conv = wn(nn.ConvTranspose2d(in_planes, out_planes, kernel_size=kernel_size, stride=stride,
