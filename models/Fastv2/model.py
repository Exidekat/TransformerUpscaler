...
Fixes applied:
1) ShiftedWindowTransformerBlock uses (H_t, W_t) for window_reverse.
2) super() calls corrected (ResidualHABBlock).
3) residual_conv now permutes x to (B, C, H, W) before conv, then back.
4) ratio check uses out.shape[2], out.shape[3].
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        # Round up to the next even number (if needed)
        # e.g. from 45->46, 81->82
        H_even = (H + 1) if (H % 2 == 1) else H
        W_even = (W + 1) if (W % 2 == 1) else W
        pad_h = H_even - H  # bottom pad
        pad_w = W_even - W  # right pad

        if pad_h > 0 or pad_w > 0:
            # (left, right, top, bottom) => (0, pad_w, 0, pad_h)
            x = F.pad(x, (0, pad_w, 0, pad_h))
            # recapture shape after pad
            B, H, W, C = x.shape
            # Now guaranteed H%2==0, W%2==0

        # Now safe to do merges
        x0 = x[:, 0::2, 0::2, :]
        x1 = x[:, 1::2, 0::2, :]
        x2 = x[:, 0::2, 1::2, :]
        x3 = x[:, 1::2, 1::2, :]
...
class WindowAttention(nn.Module):
    def __init__(self, dim: int, window_size: int, num_heads: int, reduction_ratio: int = 2, dropout: float = 0.0):
        ...
        assert self.reduced_dim * reduction_ratio == dim, "dim must be divisible by reduction_ratio"
        assert self.reduced_dim % num_heads == 0, "reduced_dim must be divisible by num_heads"
...
class ShiftedWindowTransformerBlock(nn.Module):
    def __init__(self, dim, window_size, num_heads,
                 mlp_ratio=4.0, reduction_ratio=2,
                 dropout=0.1, shift_size=0):
        super().__init__()
        self.window_size = window_size
...
class CoarseGlobalAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        kernel = min(self.pool_size, H, W)
        pooled_x = F.avg_pool2d(x_ch, kernel_size=kernel, stride=kernel)  # (B, C, H//pool, W//pool)

        H_p, W_p = pooled_x.shape[2], pooled_x.shape[3]
        N_p = H_p * W_p
        ...
        # (B, N_p, C) -> (B, C, H_p, W_p) -> upsample -> (B, C, H, W)
        attn_out = attn_out.transpose(1, 2).view(B, C, H_p, W_p)
        attn_out_upsampled = F.interpolate(
            attn_out, size=(H, W), mode='bilinear', align_corners=False
        )
        attn_out_upsampled = attn_out_upsampled.permute(0, 2, 3, 1)
...
class TransformerModel(nn.Module):
    ...
    def __init__(
        ...
        window_size: int = 8,
        reduction_ratio: int = 2,
        coarse_attention_pool_size: int = 8
    ):
        super().__init__()
        ...
        self.body = nn.Sequential(
            *[ResidualHABBlock(dim=transformer_dim, window_size=window_size, num_heads=num_heads,
                               mlp_ratio=mlp_ratio, reduction_ratio=reduction_ratio,
                               dropout=dropout, channel_reduction=coarse_attention_pool_size,
